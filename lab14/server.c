...
int main(int argc, char* argv[])
{
    ...
    if (bind(mysocket, (struct sockaddr_in*)&local_addr,
            sizeof(local_addr))) {
        printf("Error bind %d\n", WSAGetLastError());
    ...
    while ((client_socket = accept(mysocket, (struct sockaddr_in*)&client_addr, &client_addr_size))) {
        struct hostent* hst;
        int bytes_recv;
        ...
        while ((bytes_recv = recv(client_socket, &buff[0], sizeof(buff), 0)) && bytes_recv != SOCKET_ERROR){
            printf("> ");
            fgets(&buff2[0], sizeof(buff2), stdin);
